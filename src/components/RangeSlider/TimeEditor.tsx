...
function TimeEditor(props: {
	...
	function setNewTime(nextValue: number, prevValue: number, multiplier: number) {
	...
	const fields: { label: string; value: number; multiplier: number; clampHour: boolean }[] = [
			...
			{fields.map((f) => (
				<TextField
					...
					onChange={(e) => {
						...
						if (f.clampHour && (raw as unknown as number) * 3600 > props.audioRef.duration) {
							setIsError(true);
