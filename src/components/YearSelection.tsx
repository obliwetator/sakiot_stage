...
export function YearSelection() {
	...
	return (
		<Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, width: '100%' }}>
			{isDesktop ? (
				<Box sx={{ flex: '0 0 20%', minWidth: 220, maxWidth: 320, overflow: 'auto' }}>{tree}</Box>
			) : (
				<Box sx={{ p: 1 }}>
					<Button variant="outlined" fullWidth startIcon={<FolderOpenIcon />} onClick={() => setTreeOpen(true)}>
						Browse files
					</Button>
