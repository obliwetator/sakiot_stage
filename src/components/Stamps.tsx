...
export function Stamps() {
	...
	if (isLoading) {
		return <Box p={3}><Typography>Loading stamps…</Typography></Box>;
	}

	if (isError) {
		return (
			<Box p={3}>
				<Typography color="error">Failed to load stamps: {JSON.stringify(error)}</Typography>
			</Box>
		);
	}
	...
	return (
		<Box sx={{ p: 3, maxWidth: 1400 }}>
			...
			<Typography color="text.secondary" sx={{ mb: 2 }}>
				{rows.length} stamp{rows.length === 1 ? '' : 's'} (newest first, max 500).
			</Typography>
							...
							{rows.map((s) => (
								<TableRow key={s.id} hover>
									...
									<TableCell>
										...
										{s.target_name && (
											<div style={{ fontSize: 11, opacity: 0.6 }}>{s.target_user_id}</div>
										)}
									</TableCell>
									<TableCell>
										<div>{s.stamper_name ?? s.stamper_user_id}</div>
										{s.stamper_name && (
											<div style={{ fontSize: 11, opacity: 0.6 }}>{s.stamper_user_id}</div>
										)}
									</TableCell>
									<TableCell>
										<div>{s.channel_name ?? s.channel_id}</div>
										{s.channel_name && (
											<div style={{ fontSize: 11, opacity: 0.6 }}>{s.channel_id}</div>
										)}
									</TableCell>
